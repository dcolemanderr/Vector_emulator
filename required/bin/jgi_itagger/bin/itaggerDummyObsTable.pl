...
if($fasta){
	my $fasta_db = iTagger::FastaDb->new($fasta) or die("Unable to open Fasta file, $fasta\n");
	...
	my $fastq_db = iTagger::FastqDb->new($fastq) or die("Unable to open Fasta file, $fastq\n");
