...
same order. For instance Preprocess454.pl \
...
foreach my $fasta (@fasta){
	...
	my $in_1 = new iTagger::FastqDb($infile, {trim3=>0, trimN=>0}) or die("Unable to open Fastq file, $infile\n");
...
foreach my $file (@original_indexes){
	...
	my $in = new iTagger::FastqDb($fastq, {trim3=>0, trimN=>0}) or die("Unable to open Fastq file, $fastq\n");
